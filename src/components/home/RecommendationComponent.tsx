...
const recommendations = [
    {
        ...
        time: "30 min",
        sold: "200+",
        imageUrl: "/category/vegatable.png"
    },
    {
        id: 2,
        title: "Healthy Taco Salad with fresh",
                ...
                {recommendations.map((recommendation) => (
                    <div key={recommendation.id}
                        ...
                        <div className="flex flex-col gap-1 justify-start">
                            <p className="text-base font-light mt-2">{recommendation.title}</p>
                            <div className="flex justify-start items-center gap-1">
                                <p className="font-light text-gray-500 text-sm">{recommendation.price}</p>
                                <span
                                    className="w-1 h-1 rounded-full bg-accent"></span>
                                <p className="font-light text-gray-500 text-sm">{recommendation.category}</p>
                            </div>
                            <div className="flex items-center gap-2">
                                <div className="flex justify-start items-center gap-1">
                                    <GoClock className="text-gray-500 w-[14px] h-[14px]"/>
                                    <p className="font-light text-gray-500 text-sm">{recommendation.time}</p>
                                </div>
                                <span className="w-1 h-1 rounded-full bg-primary"></span>
                                <div className="flex justify-start items-center gap-1">
                                    <HiOutlineFire className="text-gray-500 w-[14px] h-[14px]"/>
                                    <p className="font-light text-gray-500 text-sm">បានលក់</p>
                                    <p className="font-light text-gray-500 text-sm">{recommendation.sold}</p>
                                </div>
                            </div>
                        </div>
                    </div>
